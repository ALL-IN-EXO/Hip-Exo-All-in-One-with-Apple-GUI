"""
LSTMNetworkLegDcp - LSTM神经网络控制器 (每条腿独立解耦)
...
class LSTMNetworkLegDcp(nn.Module):
    ...
    def generate_assistance(self, L_IMU_angle, R_IMU_angle, L_IMU_Vel, R_IMU_Vel):
        ...
        self.hip_torque_L = 0.2 * action[0] * self.kp + self.dqTd_L * self.kd
        self.hip_torque_R = 0.2 * action[1] * self.kp + self.dqTd_R * self.kd
        ...
        # P control
        self.L_p = 0.2 * action[0] * self.kp
        self.R_p = 0.2 * action[1] * self.kp
