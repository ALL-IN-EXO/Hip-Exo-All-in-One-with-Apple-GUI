...
| 33 | eg_legacy_flags | uint8 | EG Legacy A/B flags: bit0=legacy delay scaling, bit1=gate uses x_prev, bit2=internal LPF |
...
|---|---|---|---|---|---|---|---|---|
| EG（Teensy native） | Teensy `Controller_EG` | Teensy `AutoDelayOptimizer`（grid） | Teensy（EG post-delay L/R） | 默认：Teensy 主循环统一 2阶 Butterworth（电机前）；`eg_legacy_flags.bit2=1` 时改为 EG 内部一阶 LPF(0.85/0.15) + 主循环旁路 | Raw | Physical | 优先 Teensy v3 status（经 `rpi_uplink_buf` 上行）；无 status 时回退本地 strip 窗口计算 | GUI: `GUI_RL_update/data/*.csv`；Teensy SD: `walking_log_*.csv` |
...
flowchart LR
    ...
    subgraph NATIVE["Teensy-native algorithms (EG/Samsung/SOGI/Test)"]
      ...
      C1 --> C3F{"EG legacy internal LPF?"}
      C3F -- "No" --> C3A["Unified pre-motor LPF (2nd BW, cutoff configurable)"]
      C3F -- "Yes (bit2)" --> C3B["EG internal LPF (0.85/0.15), bypass unified LPF"]
...
- EG 额外支持 Legacy A/B：当 `eg_legacy_flags.bit2=1` 时，启用 EG 内部 LPF（0.85/0.15）并旁路主循环统一 LPF，避免双滤波。
