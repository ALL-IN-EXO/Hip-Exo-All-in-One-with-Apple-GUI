...
| 31-32 | input_filter_fc_hz | int16 | Teensy 输入滤波 Butterworth 截止频率 ×100 (Hz) |
| 33 | legacy_flags | uint8 | bit0..2=EG legacy path，bit3=Samsung legacy internal LPF placeholder |
| 34-35 | torque_filter_fc_hz | int16 | Teensy 统一“电机前”滤波 Butterworth 截止频率 ×100 (Hz) |
| 5-57 | algo_params | varies | 算法专用参数 (53B, 根据 algo 解析) |
| **58-97** | **RPi passthru** | **raw** | **RPi 透传区 (40B)** |
| 98 | filter_flags | uint8 | bit0=input enable, bit1=torque enable, bit2=input butterworth, bit3=torque butterworth |
| 99-100 | input_filter_alpha | int16 | 输入滤波 LPF alpha ×1000 |
| 101-102 | torque_filter_alpha | int16 | 力矩前滤波 LPF alpha ×1000 |
| 103-124 | reserved | - | 预留（GUI→Teensy 下行） |
...
| `pf_imu` | PF-IMU (粒子滤波控制器) | 4 | `tau = clamp(b*dg_int + d*(dg_int-dg))` | 在线参数见 `PF_IMU_DEPLOYMENT.md` |
| `myoassist_1966080` | NJIT MyoAssist ZIP | 18 (2x输入历史 + 当前 + 3x输出历史) | `tau_R/L = action[0/1] * tau_max` | `tau_max=15`, scale=1.0, delay=0ms |
| `myoassist_2293760` | NJIT MyoAssist ZIP | 18 (同上) | `tau_R/L = action[0/1] * tau_max` | `tau_max=15`, scale=1.0, delay=0ms |
...
| | DNN | LSTMNetwork | LSTMNetworkLegDcp | LSTMNetworkPD | PF-IMU | MyoAssist |
|---|---|---|---|---|---|---|
| **NN_TYPE_CODE** | 0 | 1 | 2 | 3 | 4 | 5/6 |
| **结构** | 18→128→64→2 前馈 | 4→256(×2层)→2 LSTM | 每腿 2→256(×2层)→1 LSTM | 每腿 2→256(×2层)→1 LSTM | 粒子滤波 + 置信度门控 | 18→128→64→2(Tanh) + 对称交换 |
| **滤波器（当前实现）** | DNN 可用 vel/ref 运行时滤波；torque 滤波统一在主循环 | 内部 exo_filter 旁路；torque 滤波统一在主循环 | 内部 exo_filter 旁路；torque 滤波统一在主循环 | 内部 exo_filter 旁路；torque 滤波统一在主循环 | 内部 exo_filter 旁路；torque 滤波统一在主循环 | 内部 torque 滤波关闭；torque 滤波统一在主循环 |
| **IMU符号** | 取反 (`INVERT_IMU_SIGN=True`) | 不取反 | 不取反 | 不取反 | 不取反 | 不取反 |
| **模型路径** | `models/dnn/Trained_model3.pt` | `models/lstm/end2end/...` | `models/lstm/end2end/walkv2_legdecp/` | `models/lstm/end2end/walkv2_pd/` | 无固定权重 | `models/njit/model_*.zip` |

### 10.4 运行时滤波器调节 (GUI → RPi)

通过 BLE 透传区 (40B) 从 GUI 发送运行时配置（RPi 解析点：`parse_runtime_cfg()`）：
- **Torque（所有 NN）**: 主循环统一滤波器 `type/cutoff/order/enable`
- **Input（DNN / PF-IMU）**: `enable_mask` 的 `vel/ref` 位决定输入滤波是否开启
- **Auto Delay**: `auto_flags` 控制开关和方法（Grid / BO）
...
RPi 默认每 `50` 帧（约 `0.5s @100Hz`）发送状态帧（header `AA 56` + 40B）：
```
[0-1]  magic: 'R' 'L' (0x52 0x4C)
[2]    version: 0x03   (v3: per-leg int16 packed)
[3]    nn_type: 0=DNN, 1=LSTM, 2=LSTMLegDcp, 3=LSTMNetworkPD, 4=PF-IMU, 5=MyoAssist-1966, 6=MyoAssist-2293
...
[28..29]  pos_per_s_L     int16 ×100  (W)
[30..31]  pos_per_s_R     int16 ×100
[32..33]  neg_per_s_L     int16 ×100  (W, <=0)
[34..35]  neg_per_s_R     int16 ×100
[36..37]  best_delay_L    int16 ×10   (0.1ms)
[38..39]  best_delay_R    int16 ×10
```

### 10.5.1 Power 相关“字段来源”对照（你最关心）

RPi `AA56` 状态帧主要承载的是 **Eval 指标**（`ratio/+P/-P/best_delay`），不是实时 `Power Sign` 波形值。  
GUI 上的 `Physical/Control` 实时功率来自 BLE 上行的 `telemetry_ext[101..127]`：

```
telemetry_ext (27B) at BLE payload [101..127]
[101..102] magic 'X''T'
[103]      version (=1)
[104]      flags
[105..106] sample_id
[107..108] phys_pwr_L  int16 (W*100, Teensy 物理功率)
[109..110] phys_pwr_R  int16
[111..112] sync_ang_L  int16 (deg*100)
[113..114] sync_ang_R  int16
[115..116] sync_vel_L  int16 (deg/s*10)
[117..118] sync_vel_R  int16
[119..120] sync_cmd_L  int16 (Nm*100)
[121..122] sync_cmd_R  int16
[123..124] ctrl_pwr_L  int16 (W*100, 控制功率)
[125..126] ctrl_pwr_R  int16
[127]      age_ms_10
```

因此：
- `Power: Physical (Teensy)` 取 `phys_pwr_*`（实时物理功率）
- `Power: Control (Pi)` 取 `ctrl_pwr_*`（控制口径功率）
- `Eval`（右下角比例）取 `AA56` 的 `ratio/+P/-P` 指标
...
|---|---|---|---|---|---|---|---|---|
| EG（Teensy native） | Teensy `Controller_EG` | Teensy `AutoDelayOptimizer`（grid） | Teensy（EG post-delay L/R） | 输入端：可选 Input Filter（LPF/BW）；`legacy bit2` ON 时输入滤波旁路并走 EG 内部 LPF(0.85/0.15)。输出端：统一 Torque Prefilter（LPF/BW）始终可用 | Raw | `Physical (Teensy)` | 优先 Teensy v3 status（经 `rpi_uplink_buf` 上行）；无 status 时回退本地 strip 窗口计算 | GUI: `GUI_RL_update/data/*.csv`；Teensy SD: `walking_log_*.csv` |
| Samsung（Teensy native） | Teensy `Controller_Samsung` | Teensy `AutoDelayOptimizer`（grid） | Teensy（Samsung delay） | 输入端：可选 Input Filter（LPF/BW）；`legacy bit3` ON 时输入滤波旁路（当前仅占位，算法内部LPF待实现）。输出端：统一 Torque Prefilter（LPF/BW） | Raw | `Physical (Teensy)` | 同上（优先 Teensy v3 status，缺失时本地回退） | GUI + Teensy SD |
| SOGI（Teensy native） | Teensy `Controller_SOGI` | Teensy `AutoDelayOptimizer` 仅算指标（`enabled=false`，不改 delay） | 无 auto delay 生效（固定控制链） | 输入端：可选 Input Filter（LPF/BW）；输出端：统一 Torque Prefilter（LPF/BW） | Raw | `Physical (Teensy)` | 通常走 Teensy v3 status（ratio 仅监视）；缺失时本地回退 | GUI + Teensy SD |
| Test（Teensy native） | Teensy `Controller_Test` | 无 | 无 | 输入端：可选 Input Filter（LPF/BW）；输出端：统一 Torque Prefilter（LPF/BW） | Raw | `Physical (Teensy)` | 无专用 status，通常走本地回退 | GUI + Teensy SD |
| RL + Pi 在线（sync from Pi） | RPi `RL_controller_torch.py` | RPi（grid 或 BO） | RPi（per-leg delay） | RPi 主循环统一 torque filter（默认 Butterworth 5Hz 2阶），顺序 `filter->delay->scale->send` | Sync | `Control (Pi)`（Auto）或 `Physical (Teensy)`（手动选） | `Live`=power strip 实时流；`Eval`=Pi `AA56` 状态（经 Teensy 透传） | GUI + RPi `RPi_Unified/output/*.csv` + Teensy SD |
| RL + Pi 离线/超时 | Teensy `Controller_RL` 超时回零 | 无 | 无（扭矩归零） | 无 | Raw（sync 无效回退） | `Physical (Teensy)`（有则用）否则 None | 通常无有效 Pi 状态，走本地回退 | GUI + Teensy SD（Pi 日志可能中断） |
...
flowchart LR
    ...
    subgraph NATIVE["Teensy-native algorithms (EG/Samsung/SOGI/Test)"]
      B --> C0{"Input Filter stage"}
      C0 -- "Enabled" --> C0A["Input Filter (LPF or Butterworth)"]
      C0 -- "Bypassed (RL / EG-legacy / Samsung-legacy)" --> C0B["Raw IMU input"]
      C0A --> C1["Controller_* compute torque"]
      C0B --> C1
      C1 --> C2["(EG/Samsung/SOGI) AutoDelayOptimizer tick@1Hz"]
      C1 --> C3A["Unified torque prefilter (LPF/BW, Teensy-local)"]
      C3A --> C3["Motor CAN cmd"]
...
2. `Power:Auto`
- `active_algo == RL` 且 `telem_ctrl_valid && sync_from_pi`：显示 `Control (Pi)`（Pi 控制口径）。
- 其余情况：显示 `Physical (Teensy)`（Teensy 物理口径）；若无效则 `None`。
- GUI 的 `Control` 明确绑定 Pi 同步来源（`sync_from_pi`）；非 Pi 同步不会在 Auto 下选 Control。
...
4. Motor 方向按钮与 Power 解耦
- `Motor: L+/R+` 只影响下发给执行器的扭矩方向（控制输出方向）。
- GUI 中 `Power Sign` 条带、`Live/Eval` 文本、CSV 记录的 `L_pwr_W/R_pwr_W` 不再乘电机方向位。
- `Visual: VL+/VR+` 仍然只影响 Cmd/Est 的可视化符号，不影响功率。
...
2. Teensy-native
- EG/Samsung/SOGI/Test：在 Teensy 主循环提供两级可选滤波：
  - 输入滤波（角度+速度）：LPF(alpha) / Butterworth(fc) 二选一，可关闭。
  - 力矩前统一滤波（电机前）：LPF(alpha) / Butterworth(fc) 二选一，可关闭。
- EG Legacy（`legacy_flags.bit2=1`）与 Samsung Legacy（`legacy_flags.bit3=1`）只覆盖输入链路：输入滤波旁路；力矩前统一滤波不旁路。
- RL：显式旁路 Teensy 力矩前统一滤波，保持 Pi 回传扭矩透明下发。
...
- Teensy SD CSV：本地执行链路与物理量（含 `L_pwr_W/R_pwr_W`、`gait_freq_Hz/gait_period_ms` 与统一时间轴字段）。
